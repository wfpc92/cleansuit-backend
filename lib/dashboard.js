...
dashApp.controller('username', ['$scope', 'userid', 'Restangular', '$http', '$interval', '$location', '$window', 'notification',
  function($scope, userid, Restangular, $http, $interval, $location, $window, notification) {
    var fecha = null;
    ...
    $interval(function() {
      $http({
        ...
        params: {
          estado: 'nueva',
          noleido: true,
          fecha__gte: fecha ? fecha.toISOString() : null
        }
      }).then(
        response => {
          if (response.data.count > 0) {
            if (fecha && $location.path() == '/dashboard') {
              $window.location.reload();
            } else {
              notification.log(`Hay ${ response.data.count } ordenes sin atender!`, { addnCls: 'humane-flatty-info', timeout: 37000, clickToClose: true })
            }
          }
          fecha = moment.utc();
        },
...
dashApp.controller('ModalInstanceCtrl', ['$scope', '$uibModalInstance', function ($scope, $uibModalInstance, msg) {
  $scope.msg = msg;
...
dashApp.config(['NgAdminConfigurationProvider', function(nga) {
  ...
  var admin = nga.application('Panel | CleanSuit', false)
