module.exports = function(nga, admin) {
  return nga.menu()
    .addChild(nga.menu(admin.getEntity('ordenes'))
      .title('Ordenes')
      .icon('<span class="fa fa-shopping-cart fa-fw"></span>')
      .addChild(nga.menu()
        .title('Todas')
        .link('/ordenes/list?sortField=ordenes_ListView.codigo&sortDir=DESC')
        .icon('<span class="fa fa-list fa-fw"></span>'))
      .addChild(nga.menu()
        .title('En Cola')
        .link('/ordenes/list?search={"estado":"nueva"}')
        .icon('<span class="fa fa-hourglass fa-fw"></span>'))
      .addChild(nga.menu()
        .title('En Recolección')
        .link('/ordenes/list?search={"estado":"rutaRecoleccion"}')
        .icon('<span class="fa fa-car fa-fw"></span>'))
      .addChild(nga.menu()
        .title('Recolectada')
        .link('/ordenes/list?search={"estado":"recolectada"}')
        .icon('<span class="fa fa-check-square-o fa-fw"></span>'))
      .addChild(nga.menu()
        .title('En Proceso')
        .link('/ordenes/list?search={"estado":"procesando"}')
        .icon('<span class="fa fa-share-square-o fa-fw"></span>'))
      .addChild(nga.menu()
        .title('En Entrega')
        .link('/ordenes/list?search={"estado":"rutaEntrega"}')
        .icon('<span class="fa fa-truck fa-fw"></span>'))
      .addChild(nga.menu()
        .title('Finalizadas')
        .link('/ordenes/list?search={"estado":"entregada"}')
        .icon('<span class="fa fa-check fa-fw"></span>'))
      .addChild(nga.menu()
        .title('Canceladas')
        .link('/ordenes/list?search={"estado":"cancelada"}')
        .icon('<span class="fa fa-close fa-fw"></span>'))
    )
    .addChild(nga.menu()
      .title('Facturación')
      .link('/facturas/list')
      .icon('<span class="fa fa-usd fa-fw"></span>')
    )
    .addChild(nga.menu()
      .title('Promociones')
      .icon('<span class="fa fa-shopping-bag fa-fw"></span>')
      .addChild(nga.menu()
        .title('Listado')
        .link('/promociones/list')
        .icon('<span class="fa fa-list fa-fw"></span>'))
      .addChild(nga.menu()
        .title('Agregar Promoción')
        .link('/promociones/create')
        .icon('<span class="fa fa-shopping-basket fa-fw"></span>'))
    )
    .addChild(nga.menu(admin.getEntity('servicios'))
      .icon('<span class="fa fa-phone fa-fw"></span>')
      .addChild(nga.menu()
        .title('Listado')
        .link('/servicios/list')
        .icon('<span class="fa fa-list fa-fw"></span>'))
      .addChild(nga.menu()
        .title('Agregar Servicio')
        .link('/servicios/create')
        .icon('<span class="fa fa-plus fa-fw"></span>'))
      .addChild(nga.menu()
        .title('Subservicios')
        .link('/subservicios/list')
        .icon('<span class="fa fa-list fa-fw"></span>'))
      .addChild(nga.menu()
        .title('Agregar Subservicio')
        .link('/subservicios/create')
        .icon('<span class="fa fa-plus fa-fw"></span>'))
    )
    .addChild(nga.menu(admin.getEntity('productos'))
      .icon('<span class="fa fa-industry fa-fw"></span>')
      .addChild(nga.menu()
        .title('Listado')
        .link('/productos/list')
        .icon('<span class="fa fa-list fa-fw"></span>'))
      .addChild(nga.menu()
        .title('Agregar Producto')
        .link('/productos/create')
        .icon('<span class="fa fa-plus fa-fw"></span>'))
    )
    .addChild(nga.menu()
      .title('Inventario')
      .icon('<span class="fa fa-cubes fa-fw"></span>')
      .addChild(nga.menu()
        .title('Listado')
        .link('/produmoves/list')
        .icon('<span class="fa fa-list fa-fw"></span>'))
      .addChild(nga.menu()
        .title('Registrar Movimiento')
        .link('/produmoves/create')
        .icon('<span class="fa fa-retweet fa-fw"></span>'))
    )
    // .addChild(nga.menu()
    //   .title('Informes')
    //   .icon('<span class="fa fa-dashboard fa-fw"></span>')
    //   .addChild(nga.menu()
    //     .title('Facturación')
    //     .icon('<span class="fa fa-line-chart fa-fw"></span>'))
    //   .addChild(nga.menu()
    //     .title('Servicios')
    //     .icon('<span class="fa fa-area-chart fa-fw"></span>'))
    //   .addChild(nga.menu()
    //     .title('Productos')
    //     .icon('<span class="fa fa-bar-chart fa-fw"></span>'))
    // )
    .addChild(nga.menu()
      .title('Usuarios')
      .link('/usuarios/list')
      .icon('<span class="fa fa-users fa-fw"></span>')
      .addChild(nga.menu()
        .title('Listado')
        .link('/usuarios/list')
        .icon('<span class="fa fa-list fa-fw"></span>'))
      .addChild(nga.menu()
        .title('Crear Usuario')
        .link('/usuarios/create')
        .icon('<span class="fa fa-user-plus fa-fw"></span>'))
      // .addChild(nga.menu()
      //   .title('Segments')
      //   .link('/segments') // this state isn't handled by ng-admin - no problem
      //   .active(path => path == '/segments')
      //   .icon('<span class="fa fa-scissors fa-fw"></span>'))
    )
    .addChild(nga.menu()
      .title('Configuración')
      .link('/settings/edit/57c99bbf6e081dc454d9b0e9')
      .icon('<span class="fa fa-cog fa-fw"></span>')
    )
    .addChild(nga.menu()
      .template('<a class="visible-xs" href="/salir"><span class="fa fa-sign-out fa-fw"></span> Salir</a>')
    );
}
