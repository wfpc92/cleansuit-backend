function basket() {
  ...
  return {
    ...
    controller: ['$scope', '$filter', function($scope, $filter) {
      ...
      $scope.orden['orden.abono'] = parseFloat($scope.orden['orden.abono'] ? $scope.orden['orden.abono'] : 0);
      $scope.orden['orden.totales.domicilio'] = parseFloat($scope.orden['orden.totales.domicilio'] ? $scope.orden['orden.totales.domicilio'] : 0);
      $scope.orden['orden.totales.impuestos'] = parseFloat($scope.orden['orden.totales.impuestos'] ? $scope.orden['orden.totales.impuestos'] : 0);
      $scope.tracking = []
      if ($scope.editable) {
          ...
          if (['nueva', 'rutaRecoleccion', 'cancelada'].indexOf(orden.estado) != -1) {
            for (i in orden.items) {
              if (orden.items.hasOwnProperty(i)) {
                ...
                // calcula subtotal
                orden['orden.totales.subtotal'] += parseFloat(orden.items[i].precio * orden.items[i].cantidad);
              }
            }
          } else {
            if (orden.recoleccion.items.productos) {
              for (i in orden.recoleccion.items.productos) {
                if (orden.recoleccion.items.productos.hasOwnProperty(i)) {
                  ...
                  // calcula subtotal
                  orden['orden.totales.subtotal'] += parseFloat(orden.recoleccion.items.productos[i].precio * orden.recoleccion.items.productos[i].cantidad);
                }
              }
            }
            if (orden.recoleccion.items.prendas) {
              ...
              for (i in orden.recoleccion.items.prendas) {
                if (orden.recoleccion.items.prendas.hasOwnProperty(i)) {
                  ...
                  // calcula subtotal
                  orden['orden.totales.subtotal'] += parseFloat(orden.recoleccion.items.prendas[i].subservicio.precio);
                }
              }
            }
          }
...
<tr ng-if="['nueva', 'rutaRecoleccion', 'cancelada'].indexOf(orden.estado) == -1" ng-repeat="item in orden.recoleccion.items.prendas">
    ...
    <td>{{ item.codigo }}</td>
    <td><button ng-if="item.novedades" uib-popover-html="tracking[$index]" popover-trigger="outsideClick" class="btn btn-default" type="button">Tracking</button></td>
    <td class="ng-admin-type-amount">
