            ...
            modalInstance = $uibModal.open({
              ...
              template: `
                <div class="modal-header">
                  <h3 class="modal-title" id="modal-title">Descargar Factura</h3>
                </div>
                <div class="modal-body" id="modal-body">
                  {{ msg }}
                </div>
                <div class="modal-footer">
                  <button class="btn btn-success" type="button" ng-click="ok()">Descargar</button>
              ...
              resolve: {
                msg: function() {
                  return 'Orden #' + scope.orden.values.codigo;
                }
              },
            });
...
<a ng-if="orden.values.estado == 'recolectada'" class="btn btn-success" ng-class="size ? \'btn-outline btn-\' + size : \'btn-default\'" ng-click="status('procesando')">
    <span class="fa fa-check" aria-hidden="true"></span>&nbsp;Procesar
</a>
<a ng-if="orden.values.estado == 'procesando'" class="btn btn-success" ng-class="size ? \'btn-outline btn-\' + size : \'btn-default\'" ng-click="status('rutaEntrega')">
    <span class="fa fa-check" aria-hidden="true"></span>&nbsp;Facturar y Entregar
</a>
<a ng-if="['rutaEntrega', 'entregada'].indexOf(orden.values.estado) != -1" class="btn" ng-class="size ? \'btn-outline btn-\' + size : \'btn-default\'" ng-click="status('factura')">
    <span class="fa fa-check" aria-hidden="true"></span>&nbsp;Ver Factura
</a>
`
  };
}
